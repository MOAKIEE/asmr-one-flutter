...
- **播放器** — 后台播放与通知栏控制、队列管理、0.5x~2x 倍速、循环与随机、
  定时关闭、低音质省流量、进度自动续播、重启恢复队列（不自动播放）、睡眠音量渐弱与播完本曲停止
- **资料库（离线可用）** — 收藏、收听历史、本地播放列表、整部作品离线下载（暂停 / 续传 / 重启恢复 / 仅 Wi-Fi / 体积统计）
- **云端** — 登录后可管理云端播放列表，并解锁完整标签库与付费作品音频
- **界面** — 三语（中 / 日 / 英）、明暗主题与 8 种强调色、封面取色、悬浮迷你播放器

## 1.1.0 新增

- 播放器右上角进入「字幕与书签」：导入 UTF-8 LRC / SRT / VTT，或选择作品自带字幕；支持同步高亮、点击跳转和时间偏移。
- 时间点书签支持备注、编辑和删除；A–B 循环可重复播放指定片段。
- 设置中的「备份与恢复」导出 JSON，导入时合并收藏、历史、列表、进度、书签和字幕；不包含令牌及音频。
- 下载续传依赖服务器 Range 与文件校验标识，不支持时安全重下。任务在重启应用后恢复；不承诺进程被系统终止后仍继续下载。
- 修复存储失败静默丢数据、超过 500 条收藏不可见、清理时下载继续写入、加载失败误报成功和离线曲目顺序问题。
